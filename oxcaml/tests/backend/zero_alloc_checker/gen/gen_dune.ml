let () =
  let enabled_if =
    (* CR xclerc: on arm64, we currently get different results for a couple of
       cases (e.g. probes) *)
    (* Disable tests when RunLengthEncoding name mangling is enabled, since symbol names differ.
       The OXCAML_NAME_MANGLING environment variable is set by the Makefile based on configure. *)
