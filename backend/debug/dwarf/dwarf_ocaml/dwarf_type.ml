...
let rec type_shape_to_dwarf_die (type_shape : Type_shape.Type_shape.t)
    (type_layout : base_layout) ~parent_proto_die ~fallback_value_die =
  ...
  | None ->
    ...
    | Ts_constr ((type_uid, type_path), shapes) ->
      type_shape_to_dwarf_die_type_constructor ~reference ~name
        ~parent_proto_die ~fallback_value_die ~type_uid type_path type_layout
        shapes
    | Ts_variant fields ->
...
and type_shape_to_dwarf_die_tuple ~name ~reference ~parent_proto_die
  ...
  let fields =
    List.map
      (fun sh ->
        (* CR sspies: Tuples cannot have unboxed components, right? *)
...
and type_shape_to_dwarf_die_predef ~name ~reference ~parent_proto_die
  ...
  | Array, [element_type_shape] ->
    let element_base_layout = Jkind_types.Sort.Value in
    ...
    let child_die =
      type_shape_to_dwarf_die ~parent_proto_die ~fallback_value_die
        element_type_shape element_base_layout
    in
...
and type_shape_to_dwarf_die_type_constructor ~reference ~name ~parent_proto_die
  ...
  | Some type_decl_shape -> (
    ...
    | Tds_alias alias_shape ->
      let alias_die =
        type_shape_to_dwarf_die alias_shape type_layout ~parent_proto_die
          ~fallback_value_die
      in
      ...
      let fields =
        List.map
          (fun (name, type_shape, type_layout) ->
            let field_base_layout =
              match type_layout with
              | Layout.Base base_layout -> base_layout
              | Layout.Product _ ->
                Misc.fatal_error "Should have been eliminated by flattening."
            in
            ( name,
              Arch.size_addr,
              (* field size for values *)
              type_shape_to_dwarf_die ~parent_proto_die ~fallback_value_die
                type_shape field_base_layout ))
          fields
      in
...
and type_shape_to_dwarf_die_poly_variant ~reference ~parent_proto_die
  ...
  let constructors_with_references =
    Type_shape.Type_shape.poly_variant_constructors_map
      (fun sh ->
        let argument_layout = Jkind_types.Sort.Value in
        type_shape_to_dwarf_die ~parent_proto_die ~fallback_value_die sh
          argument_layout)
      constructors
  in
...
let rec flatten_and_unarize_shape (type_shape : Type_shape.Type_shape.t)
    (type_layout : Layout.t) : (Type_shape.Type_shape.t * base_layout) list =
  let unknown_base_layouts layout =
    let base_sorts = flatten_to_base_sorts layout in
    List.map
      (fun base_sort -> Type_shape.Type_shape.Ts_other, base_sort)
      base_sorts
  in
  let value_base_shape = Jkind_types.Sort.Value in
  match type_shape, type_layout with
  | Ts_var _, Base base_layout -> [type_shape, base_layout]
  | Ts_var _, (Product _ as type_layout) -> unknown_base_layouts type_layout
  | Ts_tuple shapes, Base Value ->
    let shapes =
      List.map
        (fun sh ->
          flatten_and_unarize_single_shape sh (Layout.Base value_base_shape))
        shapes
    in
    [Type_shape.Type_shape.Ts_tuple (List.map fst shapes), value_base_shape]
  (* tuples are only a single base layout wide *)
  | Ts_tuple _, (Product _ | Base _) ->
    Misc.fatal_error "tuple must have layout value"
  | Ts_unboxed_tuple shapes, Product lys
    when List.length shapes = List.length lys ->
    List.concat_map
      (fun (sh, ly) -> flatten_and_unarize_shape sh ly)
      (List.combine shapes lys)
  | Ts_predef _, Base base_layout -> [type_shape, base_layout]
  | Ts_predef _, Layout.Product _ ->
    Misc.fatal_error "predef must have base layout"
  | Ts_arrow _, Base Value -> [type_shape, value_base_shape]
  | Ts_arrow _, (Base _ | Product _) ->
    Misc.fatal_error "arrow must have base layout"
  | Ts_variant _, Base Value -> [type_shape, value_base_shape]
  | Ts_variant _, (Base _ | Product _) ->
    Misc.fatal_error "variant must have base layout"
  | Ts_other layout ->
...
and flatten_and_unarize_single_shape type_shape type_layout =
  match flatten_and_unarize_shape type_shape type_layout with
  | [(type_shape, layout)] -> type_shape, layout
  | _ ->
    Misc.fatal_error "flatten_and_unarize_single_shape: not a single layout"
