...
module Make(Params : sig
  ...
  and ('s, 'n) subst =
    (('s, 'n) subst_data, 's) Hash_consed.t
  ...
  let equal_subst s1 s2 = Hash_consed.equal s1 s2
  ...
  let equal_nf nf1 nf2 = Hash_consed.equal nf1 nf2
    ...
    let hash subst =
      let hash_dnf_opt = function
        ...
        | Some (Thunk (local_env, shape)) ->
          Hashtbl.hash (Hash_consed.hash local_env.subst, local_env.depth, shape.Shape.hash)
      in
      Ident.Map.fold
        (fun id dnf_opt acc ->
  ...
  and Nf_hashable : sig
    ...
    and hash_nf_desc desc =
      ...
      | NApp (nf1, nf2) ->
        Hashtbl.hash (1, Hash_consed.hash nf1, Hash_consed.hash nf2)
      | NAbs (local_env, v, t, dnf) ->
        Hashtbl.hash (2, Hash_consed.hash local_env.subst, local_env.depth,
      ...
      | NProj (nf, item) -> Hashtbl.hash (5, Hash_consed.hash nf, Hashtbl.hash item)
      | NLeaf -> 6
      | NComp_unit s -> Hashtbl.hash (7, Hashtbl.hash s)
      | NError s -> Hashtbl.hash (8, Hashtbl.hash s)
      | NMu (rv, nf) -> Hashtbl.hash (9, Shape.Rec_var_ident.hash rv, Hash_consed.hash nf)
      | NRec_var rv -> Hashtbl.hash (10, Shape.Rec_var_ident.hash rv)
      | NMutrec defs ->
        Hashtbl.hash (11, Ident.Map.fold (fun id nf acc ->
          Hashtbl.hash (Ident.hash id, Hash_consed.hash nf, acc)) defs 0)
      | NProj_decl (nf, id) ->
        Hashtbl.hash (12, Hash_consed.hash nf, Ident.hash id)
      | NConstr (id, args) ->
        Hashtbl.hash (13, Ident.hash id, List.fold_left (fun acc nf ->
          Hashtbl.hash (Hash_consed.hash nf, acc)) 0 args)
      | NTuple args ->
        ...
        Hashtbl.hash (18, List.fold_left (fun acc c ->
          Hashtbl.hash (Hashtbl.hash c.pv_constr_name,
            List.fold_left (fun acc nf ->
              Hashtbl.hash (Hash_consed.hash nf, acc)) 0 c.pv_constr_args,
            acc)) 0 constrs)
      | NVariant constrs ->
      ...
      | NAt_layout (nf, layout) ->
        Hashtbl.hash (23, Hash_consed.hash nf, Hashtbl.hash layout)
    ...
    and equal_nf_desc d1 d2 =
      ...
      | NApp (v1, t1), NApp (v2, t2) ->
        if Hash_consed.equal v1 v2 then Hash_consed.equal t1 t2
        else false
      | NLeaf, NLeaf -> true
      ...
      | NMu (rv1, nf1), NMu (rv2, nf2) ->
        Shape.Rec_var_ident.equal rv1 rv2 && Hash_consed.equal nf1 nf2
      | NRec_var rv1, NRec_var rv2 -> Shape.Rec_var_ident.equal rv1 rv2
      | NMutrec defs1, NMutrec defs2 ->
        defs1 == defs2 || Ident.Map.equal Hash_consed.equal defs1 defs2
      | NProj_decl (nf1, id1), NProj_decl (nf2, id2) ->
        Ident.equal id1 id2 && Hash_consed.equal nf1 nf2
      | NConstr (id1, args1), NConstr (id2, args2) ->
        Ident.equal id1 id2 && List.equal Hash_consed.equal args1 args2
      | NTuple args1, NTuple args2 ->
        List.equal Hash_consed.equal args1 args2
      | NUnboxed_tuple args1, NUnboxed_tuple args2 ->
        List.equal Hash_consed.equal args1 args2
      | NPredef (p1, args1), NPredef (p2, args2) ->
        Predef.equal p1 p2 && List.equal Hash_consed.equal args1 args2
      | NArrow, NArrow -> true
      | NPoly_variant constrs1, NPoly_variant constrs2 ->
        let equal_pv_constructor c1 c2 =
          ...
          List.equal Hash_consed.equal c1.pv_constr_args c2.pv_constr_args
        in
      ...
      | NAt_layout (nf1, layout1), NAt_layout (nf2, layout2) ->
        Hash_consed.equal nf1 nf2 && Layout.equal layout1 layout2
  ...
  and Subst_table : sig
    type tbl
    val create : Subst_hashable.t -> (Subst_hashable.t, tbl) Hash_consed.t
  end = Hash_consed.Table (Subst_hashable)

  and Nf_table : sig
    type tbl
    val create : Nf_hashable.t -> (Nf_hashable.t, tbl) Hash_consed.t
  end = Hash_consed.Table (Nf_hashable)

  let approx_nf nf =
    let nf_data = Hash_consed.value nf in
  ...
  module ReduceMemoTable = Hashtbl.Make(struct
      ...
      let hash (local_env, shape) =
        Hashtbl.hash
          ( Hash_consed.hash local_env.subst,
            local_env.depth,
            shape.Shape.hash )
  ...
  module ReadBackMemoTable = Hashtbl.Make(struct
      ...
      let hash nf = Hash_consed.hash nf
  ...
  let bind env var shape =
    let subst_data = Hash_consed.value env.local_env.subst in
  ...
  *)
  ...
  and reduce__
    ...
    let reset_uid_if_new_binding t' =
      ...
      | None -> t'
      | Some _ as uid ->
        let t' = Hash_consed.value t' in
        Nf_table.create { t' with uid }
    in
    let set_uid_if_none uid t =
      let t_data = Hash_consed.value t in
    ...
    then return NUnknown_type
      ...
      | App(f, arg) ->
          ...
          let f_data = Hash_consed.value f in
      ...
      | Proj(str, item) ->
          ...
          let str_data = Hash_consed.value str in
      ...
      | Var id ->
          let subst_data = Hash_consed.value local_env.subst in
