...
let rec estimate_layout_from_type_shape (t : Shape.t) :
    Layout.t option =
  match t.desc with
  | Predef (t, _) -> Some (Shape.Predef.to_layout t)
  | Constr (_, _) -> None (* recursive occurrence, conservatively not
                             handled for now *)
  ...
  | Variant_unboxed { arg_layout; _ } -> Some arg_layout
    (* CR sspies: [arg_layout] could become unreliable in the future. Consider
       recursively descending in that case. *)
  | Tuple _ | Arrow _ | Variant _ | Poly_variant _ | Record _ -> Some (Layout.Base Value)
  | Alias t -> estimate_layout_from_type_shape t
  | Mu t -> estimate_layout_from_type_shape t
    (* Simple treatment of recursion, we simply look inside. *)
  | Leaf | Abs _ | Mutrec _ | Error _ | Comp_unit _ | Rec_var _
  | App _ | Proj _ | Struct _ | Proj_decl _ -> None
