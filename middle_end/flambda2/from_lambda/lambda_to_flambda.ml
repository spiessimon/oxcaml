...
let rec cps acc env ccenv (lam : L.lambda) (k : cps_continuation)
  ...
  | Lmutvar id ->
    ...
    in
    let fields =
      List.map (fun (id, _, _) -> IR.Var id) new_ids_with_kinds
    in
    apply_cps_cont_simple k acc env ccenv fields before_unarization
  | Lconst const ->
  ...
  | Lmutlet (layout, id, duid, defining_expr, body) ->
    ...
    let_cont_nonrecursive_with_extra_params acc env ccenv ~is_exn_handler:false
      ~params:[temp_id, duid, IR.Not_user_visible, layout]
      (* CR sspies: Probably not where we want the duid to end up. *)
      ...
      ~handler:(fun acc env ccenv ->
        ...
        let temp_id_unarized : Ident.t list =
          ...
          (* CR sspies: Probably not what we want to do here. *)
        in
        List.fold_left2
          ...
          let env, ids_with_kinds =
            ...
            | Punboxed_product layouts ->
              ...
              let fields =
                ...
                |> Flambda_uid.add_proj_debugging_uids_to_fields ~duid
              in
              let env =
