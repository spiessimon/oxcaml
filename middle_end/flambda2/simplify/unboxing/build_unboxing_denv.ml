...
let denv_of_number_decision naked_kind shape param_var naked_var denv : DE.t =
  let naked_name = VB.create naked_var Flambda_uid.internal_not_actually_unique Name_mode.normal in
...
let rec denv_of_decision denv ~param_var (decision : U.decision) : DE.t =
    ...
    let denv =
      Misc.Stdlib.List.fold_lefti
        (fun index denv ({ epa = { param = var; _ }; _ } : U.field_decision) ->
          let v = VB.create var Flambda_uid.internal_not_actually_unique Name_mode.normal in
          (* CR tnowak: verify *)
  ...
  | Unbox (Variant { tag; const_ctors; fields_by_tag }) ->
    ...
    let tag_v = VB.create tag.param Flambda_uid.internal_not_actually_unique Name_mode.normal in
    (* CR tnowak: verify *)
    ...
    let denv =
      ...
      | At_least_one { is_int; _ } ->
        let is_int_v = VB.create is_int.param Flambda_uid.internal_not_actually_unique Name_mode.normal in
        (* CR tnowak: verify *)
    ...
    let denv, const_ctors =
      ...
      | At_least_one { ctor = Unbox (Number (Naked_immediate, ctor_epa)); _ } ->
        let v = VB.create ctor_epa.param Flambda_uid.internal_not_actually_unique Name_mode.normal in
        (* CR tnowak: verify *)
